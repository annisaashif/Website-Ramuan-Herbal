...
const diseases = [
  {
    id: 1,
    name: 'Batuk Kronis',
    description: 'Batuk yang berlangsung lebih dari delapan minggu (pada dewasa) atau empat minggu (pada anak-anak). Kondisi ini sering terkait dengan penyakit kronis seperti asma, penyakit paru obstruktif kronis (PPOK), tuberkulosis, atau GERD (gastroesophageal reflux disease) yang menyebabkan asam lambung naik ke tenggorokan dan mengiritasi saluran pernapasan.'
  },
  {
    id: 2,
    name: 'Asma',
    description: 'Asma adalah penyakit jangka panjang yang menyebabkan saluran pernapasan menyempit dan membengkak serta memproduksi lendir berlebih. Hal ini dapat mempersulit bernapas dan memicu batuk, mengi, serta sesak napas.'
  }
];
function IdentifikasiResep() {
  const { id } = useParams(); // Mengambil ID dari URL
  const selectedDisease = diseases.find(disease => disease.id === parseInt(id));
  ...
  return (
    <>
      ...
      <div className="relative">
        <img src={Logo} alt="Background with various herbs" className="w-full h-60 object-cover" />
        <div className="absolute inset-0 bg-black opacity-50"></div>
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white px-4">
          <h1 className="text-3xl font-bold">{selectedDisease ? selectedDisease.name : 'Penyakit Tidak Ditemukan'}</h1>
          <p className="mt-2 text-lg">
            {selectedDisease ? selectedDisease.description : 'Deskripsi penyakit tidak tersedia.'}
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {herbs.map((herb, index) => (
            <div key={index} className="bg-white rounded-lg shadow-lg overflow-hidden">
              <img src={herb.image} alt={`Image of ${herb.name}`} className="w-full h-48 object-cover" />
              <div className="p-4 bg-gradient-to-t from-black to-transparent text-white text-center">
